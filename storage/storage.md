...
* ``ID string `json:"id"``
* ``Type string `json:"type"``
  * _Example:_ `oidc`, `ldap`
* ``Name string `json:"name"``
  * displayed | end user
* ``ResourceVersion string `json:"resourceVersion"``
  * static versioning /
    * track dynamic configuration changes -- via -- API calls
* ``Config []byte `json:"email"``
  * TODO: configuration specific to the connector type -- stored as byte stream
  * ⚠️ JSON tag should be `config` but is `email` (known bug -- fixing requires Kubernetes object migration)
* ``GrantTypes []string `json:"grantTypes,omitempty"``
  * grant types this connector is allowed to be used with
  * if empty -> ALL grant types allowed
...
* ``VerificationKeys []VerificationKey``
  * OLD signing keys /
    * rotated
    * valid TILL validate existing signatures
* ``NextRotation time.Time``
  * next time / signing key will rotate
  * BEFORE this time,
    * implementations MUST NOT update keys

# `type DeviceRequest struct {`
* == OIDC device authorization request TILL
  * user authenticates -- via -- their user code OR
  * expiry time passes
* ``UserCode string``
  * == code / user enter | browser
* ``DeviceCode string``
  * UNIQUE device code
  * uses
    * device authentication
* ``ClientID string``
  * client ID | code is for
* ``ClientSecret string``
* ``Scopes []string``
  * device requests scope
* ``Expiry time.Time``
